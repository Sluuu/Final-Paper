...
The log of `gdp_growth` and `population` shared similar reasons. As Economic growth rates and population can vary widely, and the impact on GNI per capita might scale logarithmically rather than linearly.
...
The negative coefficient for GDP growth in low-income countries was expected however, it was not significant. For high-income countries, GDP growth shows a positive and significant impact, indicating robust economic dynamics where growth directly enhances national income levels. Also the high income countries showed a significant stronger positive relationship on labour force participation rate and trade as a share of GDP. 
From @fig-qq-low we could see that the observations were mostly separated and with multiple outliers which indicates that we have a low sample size also deviates from normality. Which shows that for the low-income countries model, it has potentially leading to skewed or biased estimates.
From @fig-qq-high the points on the Q-Q plot fall approximately along a straight line, suggesting that the residuals follow a normal distribution. Which it also has less outliers compared with the low-income model. It also furthe confirm that the residuals are normally distributed, supporting the validity of the least squares regression assumptions.

## Weaknesses and next steps

There are several weaknesses of this study, which in the low-income countries model several important variables like GDP growth, unemployment, and inflation did not show statistically significant effects. This might be due to the lack of sample sizes and the high fluctuations in the data. The second weakness was the lack of variables, the model did not include some relevant variables that affect GNI per capita, such as political stability, technology, and education levels, etc. Which both results might create biases onto the results.
